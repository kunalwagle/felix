...
- (void)widgetPerformUpdateWithCompletionHandler:(void (^)(NCUpdateResult))completionHandler {
    ...
    if ([UtilityMethods testInternetConnection]) {
        NSMutableArray *articles = [UtilityMethods getArticles:@"News"];
        Article *article = (Article*) [[NSUserDefaults standardUserDefaults] objectForKey:@"todayArticle"];
        if (article) {
            ...
            dispatch_async(articleImageQueue, ^{
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    NSLog(@"Set the image");
                    NSData *articleEncodedObject = [NSKeyedArchiver archivedDataWithRootObject:article];
                    NSData *articleImageEncodedObject = [NSKeyedArchiver archivedDataWithRootObject:self.articleImage.image];
                    [[NSUserDefaults standardUserDefaults] setObject:articleEncodedObject forKey:@"todayArticle"];
                    [[NSUserDefaults standardUserDefaults] setObject:articleImageEncodedObject forKey:@"image"];
