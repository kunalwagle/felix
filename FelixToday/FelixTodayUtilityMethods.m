...
@implementation FelixTodayUtilityMethods
...
+(NSMutableArray*)getArticles:(NSString*)section {
    ...
    while (!a) {
        ...
        a = [FelixTodayUtilityMethods removeDuplicates:a];
        ...
        //        }
        if (!a) {
            [FelixTodayUtilityMethods loadArticles:section];
        }
    }
    return a;
}
...
+(void)loadArticles:(NSString*)section {
    NSMutableArray *articles = [FelixTodayUtilityMethods setUpArticles:section];
    NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentsDirectory = [paths objectAtIndex:0]; // Get documents folder
    NSString *dataPath = [documentsDirectory stringByAppendingPathComponent:[NSString stringWithFormat:@"/%@", section]];
...
//    UIColor *colour;
//    colour = [[UIColor alloc] initWithRed:234.0/255.0 green:204.0/255.0 blue:207.0/255.0 alpha:1];
